...
class MatrixBase(RelativeLayout):
    ...
    def make_first_notepoint(self, text):
        ...
        a.tonality = self.gen_settings['scale']
        a.attach_label()
        self.add_widget(a)
    ...
    def make_first_octave(self, *args):
        if self.__class__.__name__ == 'FundMatrix':
            settings_dict = self.fund_settings
        elif self.__class__.__name__ == 'MelodyMatrix':
            settings_dict = self.melody_settings

        if self.gen_settings['scale'] == 'Freehand':
        ...
        elif self.gen_settings['easymode'] == u'0' or self.__class__.__name__ == 'FundMatrix':
    ...
    def create_next_notepoint(self, relation):
        """
        Accepts a relation, then adds one NotePoint for each extant NotePoint in the direction of the relation - unless one already exists in that position.
        """
    ...
    def create_next_octave(self, relation):
        """
        Looks at a pseudo-registry at MelodyMatrix or FundMatrix, holding the NotePoints in the first octave.  If there isn't an extant NotePoint at relation*Original Notepoint, it creates one there.
        """
