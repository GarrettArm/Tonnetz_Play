...
class MelodyMatrix(MyRelativeLayout):
    ...
    """This class is the counterpart to FundMatrix.  It also inherits \
    most of its functions from MyRelativeLayout.  It holds an arbitrary \
    number of NotePoints, which create a tone.  This tone is analagous \
    to multiplying the fundamental freq times the MelodyMatrix NotePoint's \
    ratio"""
    ...
    def rename_child_notepoints(self):
        for i in self.children:
            ...
            shift_by = self.full_scale.index(self.passed_fund_text) - \
                self.full_scale.index(self.current_fund_text)
